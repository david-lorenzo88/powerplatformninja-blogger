...
Last updated: 2026-07-28. The React management UI (Stage 2) is now **built and
verified**, open in **PR #1**. Read this before starting anything.
...
- **Stage 2** (React management UI in `ui/`): built, verified against real Azure,
  open in **[PR #1](https://github.com/david-lorenzo88/powerplatformninja-blogger/pull/1)**
  (branch `claude/powerplatformninja-blogger-ui-75a669` → `main`). **Not yet merged.**
- **GitHub**: the repo exists and is **public**. `main` carries the crew, the docs
  and the ignore rules for generated output. The UI and the run-event enrichment
  live on the PR branch until it merges.

If you are picking this up in a fresh session: the UI code is on the PR branch, not
on `main`. Check out that branch (or merge the PR) before working on `ui/`.
...
- **Merge PR #1** into `main`, then `docs/STATUS.md` and `CLAUDE.md` here become the
  state of `main` too.
